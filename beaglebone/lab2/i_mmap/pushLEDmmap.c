...
void signal_handler(int sig){
  printf("\nCtril-C pressed, cleanning up and exiting...\n");
  keepgoing = 0; 
}
...
int main(int argc, char* argv[]){
  ...
  signal(SIGINT,signal_handler); // Signal callback for Ctrl-C
  ...
  while(keepgoing){
    if(*gpio_datain & GPIO_07){ //Check whether the button is pushed 
